...
此方法可创建报表和数据集。在本练习中，仅使用数据集创建新报表。可以使用其他报表的现有数据集完成相同的过程，而不是上传新数据集。此外，如果不使用报表，工作区最佳做法建议删除不必要的文件。

### **任务 3：创建报表**

在此任务中，创建与在上一个任务中创建的 Power BI 数据集的实时连接，然后创建新的“Sales Exploration”报表。
...
1. 在“开始”功能区中，选择“获取数据”>“Power BI 数据集”。

1. 在“数据中心”窗口中，选择“我的工作区”中的“Sales Analysis”数据集，然后通过“连接”或双击加载数据集   。
